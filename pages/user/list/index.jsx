...
export default function UserManagement() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        setLoading(true)
        const response = await axios.get("https://esi-flow-back-u9go.onrender.com/users")
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(`https://esi-flow-back-u9go.onrender.com/users/${row.id}`) // Updated API endpoint
